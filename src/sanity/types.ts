...
// Variable: PAGE_QUERY
// Query: *[_type == "page" && slug.current == $slug][0]{  "slug": slug.current,  "seo": {    "title": coalesce(seo.title, title, ""),    "description": coalesce(seo.description,  ""),    "image": seo.image,    "noIndex": seo.noIndex == true  },  "content": content[hide != true]{    ...,    // Logic for the Impact Section    _type == "impactSection" => {        "countMembers": count(*[_type == "teamMember"]),        "teamMembers": teamMembers[defined(@)]->{          name,          image        },        statistics[]{          heading,          highlightedHeading,          text        }      },    // Logic for Case Studies    _type == "caseStudiesSection" => {      "caseStudies": caseStudies[defined(@)]->{        title,        excerpt,        mainImage,        "slug": slug.current,        "services": services[]->title      }    },    _type == "allCaseStudiesSection" => {      "caseStudies": *[_type == "caseStudy"]{        title,        excerpt,        mainImage,        "slug": slug.current,        "services": services[]->title      },      "services": *[_type == "service"].title    },    _type == "allMediaSection" => {      "media": *[_type == "media"]{        title,        mainImage,        "slug": slug.current,        readTime,        publishedAt,        author->{          name,          image        },        "categories": categories[]->title      },      "categories": *[_type == "category"].title    },    _type == "allTeamMembersSection" => {      "teamMembers": *[_type == "teamMember"]| order(name asc){        name,        image,        bio,        "teams":teams[]->title      },      "teams": teams[]->title    },    _type == "allTeamSection" => {      teamGroups[]{        "teamName": team->title,        "members": members[]->{          name,          image,          bio,          "teams": teams[]->title        }      }    },    _type == "allJobsSection" => {      "jobs": *[_type == "job"]{        title,        "slug": slug.current,        "department": department->title,        postedAt,        schedule,        applicationLink,        "location": location->name,        description      },    },    _type == "allTrainingsSection" => {      "trainings": *[_type == "training"]{        title,        "slug": slug.current,        publishedAt,        description      },    },    _type == "servicesSection" => {      services[]->{        title,        description,        "slug": slug.current,        icon      }    },    _type == "heroSection" => {      companies[]->{        name,        logo      }    },    _type == "companySlideshowSection" => {      companies[]->{        name,        logo      }    },    _type == "companiesSection" => {      companies[]->{        name,        website,        logo      }    },    _type == "reviewsSection" => {      reviews[]->{        name,        image,        reviewText,        position      }    },    _type == "locationsSection" => {      locations[]->{        image,        name,        slug      }    },    _type == "mediaSection" => {      media[]->{        title,        mainImage,        "slug": slug.current,        readTime,        publishedAt,        author->{          name,          image        },        "categories": categories[]->title      }    }  }}
